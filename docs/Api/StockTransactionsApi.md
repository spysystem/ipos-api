...
> \iPosExchanger\Model\CreateRecordResponse createStockTransaction($str_database, $create_or_update_stock_transaction_request)
...
try {
    $result = $apiInstance->createStockTransaction($str_database, $create_or_update_stock_transaction_request);
    print_r($result);
} catch (Exception $e) {
